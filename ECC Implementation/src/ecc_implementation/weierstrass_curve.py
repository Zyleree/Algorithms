...
class WeierstrassCurve(EllipticCurve):
    ...
    def point_doubling(self, point):
        ...
        if isinstance(self.field, PrimeField):
            ...
            if point.y.value == 0: 
                return self.identity_point 
            slope = (point.x**2 + self.a) / point.y 
        else:
    ...
    def point_negation(self, point):
        if point == self.identity_point:
            return self.identity_point
        return Point(point.x, FiniteFieldElement(-point.y.value % self.field.prime, self.field) if isinstance(self.field, PrimeField) else FiniteFieldElement(point.y.value, self.field), self)